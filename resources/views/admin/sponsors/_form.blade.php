...
<div class="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
    <div>
        <label class="{{ $label }}">Sponsor Adı</label>
        <input name="name" value="{{ old('name', $sponsor?->name) }}" required class="{{ $input }}">
    </div>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
            <label class="{{ $label }}">Web Sitesi (link)</label>
            <input name="url" value="{{ old('url', $sponsor?->url) }}" placeholder="https://…" class="{{ $input }}">
        </div>
        <div>
            <label class="{{ $label }}">Kademe</label>
            <select name="tier" class="{{ $input }}">
                @foreach (['ana'=>'Ana Sponsor','resmi'=>'Resmi Sponsor','destekci'=>'Destekçi'] as $v=>$l)
                    <option value="{{ $v }}" @selected(old('tier', $sponsor?->tier) === $v)>{{ $l }}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
            ...
            <input type="number" name="sort" value="{{ old('sort', $sponsor?->sort ?? 0) }}" min="0" class="{{ $input }}">
        </div>
        <div class="flex items-end">
            <label class="flex cursor-pointer items-center gap-2 rounded-lg border border-slate-300 px-4 py-2.5 text-sm has-[:checked]:border-brand-500 has-[:checked]:bg-brand-50">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $sponsor?->is_active ?? true)) class="rounded text-brand-600 focus:ring-brand-500"> Aktif
            </label>
        </div>
    </div>
    <div>
        <label class="{{ $label }}">Logo</label>
        @if ($sponsor?->logoUrl())<img src="{{ $sponsor->logoUrl() }}" class="mt-2 h-16 object-contain">@endif
    ...
    <div class="flex gap-3 border-t border-slate-100 pt-5">
        ...
        <a href="{{ route('admin.sponsors.index') }}" class="rounded-lg border border-slate-300 px-6 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50">İptal</a>
    </div>
</div>
